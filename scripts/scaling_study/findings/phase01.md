...
`01_crossover.png` (clean, single-setup — see note + caveat below),
`02_sync_curve.png` (regenerated from `bench_phase0a.csv` alone — the report's
default sync-curve pools all matrices' seq/zmq rows regardless of M/work, which
for the concatenated file visually contaminates the N=1 point with heavy-work
rows from other matrices; the version here isolates the intended N-sweep at
fixed M=1, work=1).

> **Crossover-figure caveat (same class as the sync-curve one above).**
> `make_report.py`'s crossover plot groups `bench_all.csv` by `(mode, work)`
> **without filtering M** — so its *seq* line pooled M∈{1,4,16,64} rows while its
> *par* line was pure M=16/W=8 (mixed-setup, not a fair single curve; that
> pooled version is preserved as `01_crossover_old.png`). The published
> `01_crossover.png` is now **regenerated from the M=16, W=8 slice only** (both
> modes share one setup) by `scripts/scaling_study/plot_crossover_clean.py`.
> **Figure setup:** F=1, N=1, **M=16 instances, W=8 workers**, zmq, local,
> 20 ticks × 3 repeats, `heavy_compute_dummy`, `work ∈ {1000, 10000, 25000,
> 50000, 100000}`. y = mean wall-time per tick (ms), min–max band over 3
> repeats; twin top axis shows per-instance step cost `c = a + b·work` in ms.
> Measured crossover (log-interp on this clean slice) = **22,800 work
> (c ≈ 3.08 ms)**; law prediction `(M−⌈M/W⌉)·c* = O_par` = **24,480 work
> (c* ≈ 3.30 ms)** — agree to ~7%.
...
- `paper_ready_sentences.md` — **reusable paper prose** (methods/results/decision
  rule/EnergyPlus mapping paragraphs, Fig. X caption, Table Y) for the crossover
  result. Provenance-flagged (measured vs cited).
...
- `01_crossover.png` — **publication figure**: seq vs par crossover, clean
  single-setup M=16/W=8 slice (regenerated by `plot_crossover_clean.py`; see
  crossover-figure caveat above). x=`work` (comma-formatted, no sci-notation),
  twin top axis = per-instance cost `c` (ms), min–max bands, crossover line.
- `01_crossover_old.png` — **superseded**: the original pooled-M crossover plot
  from `make_report.py` (seq line mixed M∈{1,4,16,64}); kept for provenance only.
- `plot_crossover_clean.py` → in `scripts/scaling_study/` — regenerates
  `01_crossover.png` from the M=16/W=8 slice of `bench_all.csv`.
