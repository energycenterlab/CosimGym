...
1. **JSON results (default, `memory_config.sink: json`):** At the end of a run, each federate writes its buffered timeseries to JSON files under `results/<scenario_name>/<sim_id>/<federation_name>/` (`<federate>_<train|test>_storage.json`).
