...
| `topology_updates_key` | `""` | Declared in the catalog as an input key for runtime JSON topology-change commands, but **not currently consumed** by `pandapower_grid.py` — setting it has no effect yet (see [`pandapower_extension.md`](future_and_TODOs/pandapower_extension.md#72-runtime-topology-changes-addremove-components) for the proposed feature) |
